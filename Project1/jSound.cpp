...
bool jSound::Initialize(HWND _hWnd)
{
	...
	if (FAILED(m_DirectSound->SetCooperativeLevel(_hWnd, DSSCL_PRIORITY)))
	{
		return false;
	}
...
bool jSound::LoadWaveFile(string _filename)
{
	// Open the wave file in binary.
	FILE* filePtr = nullptr;
	int error = fopen_s(&filePtr, _filename.c_str(), "rb");
	if (error != 0)
	{
		return false;
	}
